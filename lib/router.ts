import { FetchProgressEvent, RifleOptions, RouteChangeData } from './interfaces';
import { addToPushState, handlePopState, handleLinkClick, scrollTo } from './handlers';
...
export class Router {
  ...
  constructor(public opts?: RifleOptions) {
  ...
  private get allLinks(): (HTMLAnchorElement | HTMLAreaElement)[] {
    return Array.from(document.links).filter(
      (node) =>
        node.href.includes(document.location.origin) &&
        !node.href.includes('#') &&
